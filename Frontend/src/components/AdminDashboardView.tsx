...
import { AuthUser, Student, Professor, Subject, Classroom, TimetableSlot, NotificationItem } from '../types';
import { StorageService } from '../services/storageService';
import { generatePDFReport, generateMultiSectionPDFReport } from '../utils/pdfExport';
import { ShieldCheck, Users, UserCheck, BookOpen, MapPin, Calendar, Bell, Plus, Trash2, Edit3, Search, Download, AlertCircle, CheckCircle2, ChevronLeft, ChevronRight, X, Database, Code, FileText, CheckSquare, Square, Printer, FileSpreadsheet, Eye, Filter, ToggleLeft, ToggleRight, FileJson } from 'lucide-react';
...
export const AdminDashboardView: React.FC<AdminDashboardProps> = ({
  ...
  const [activeModule, setActiveModule] = useState<
    'overview' | 'students' | 'professors' | 'subjects' | 'classrooms' | 'timetable' | 'notifications' | 'reports'
  >('overview');
  ...
  const confirmDelete = () => {
    ...
    if (type === 'student') StorageService.deleteStudent(id);
    else if (type === 'professor') StorageService.deleteProfessor(id);
    else if (type === 'subject') StorageService.deleteSubject(id);
    else if (type === 'classroom') StorageService.deleteClassroom(id);
    else if (type === 'timetable') StorageService.deleteTimetable(id);
    else if (type === 'notification') StorageService.deleteNotification(id);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 text-center shadow-xs">
          <Users className="w-5 h-5 text-sky-600 mx-auto mb-1.5" />
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 space-y-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-3">
              <h3 className="font-bold text-sm text-sky-700">Database Connection</h3>
              ...
              <button
                onClick={() => StorageService.resetToSampleData()}
                className="bg-white hover:bg-slate-100 text-slate-800 border border-slate-300 text-xs px-4 py-2 rounded-xl font-semibold transition-all mt-2 shadow-xs"
              >
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-3">
              <h3 className="font-bold text-sm text-sky-700">Department Master PDF Export</h3>
              <p className="text-xs text-slate-600 font-medium leading-relaxed">
              ...
              <button
                ...
                className="bg-sky-500 hover:bg-sky-600 text-white text-xs font-semibold px-4 py-2 rounded-xl flex items-center space-x-2 shadow-sm transition-all mt-2"
              >
